...
public class TransactionService {
    public void saveTransaction(Transaction transaction) {
        String transactionData = transaction.getDate() + ","
    ...
    public void displayAccountStatement(User user, Account account) {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                if (data[1].equals(user.getId())
                        && data[2].equals(account.getAccountId())) {
    ...
    public void filterTransactions(User user, String filter) {
        ...
        try {
            ...
            List<String[]> filteredTransactions = lines.stream()
                    .map(line -> line.split(","))
                    ...
                    .filter(data -> {
                        ...
                        if (filter.equals("lastMonth")) {
                            return !transactionDate.isBefore(lastMonth)
                                    && !transactionDate.isAfter(endDate);
                        }

                        if (filter.equals("lastWeek")) {
                            return !transactionDate.isBefore(lastWeek)
                                    && !transactionDate.isAfter(endOfLastWeek);
                        }

                        if (filter.equals("last7days")) {
                            return !transactionDate.isBefore(last7days)
                                    && !transactionDate.isAfter(today);
                        }

                        if (filter.equals("last30days")) {
                            return !transactionDate.isBefore(last30Days)
                                    && !transactionDate.isAfter(today);
                        }
    ...
    private double getTodayTransactionTotal(String accountId, String transactionType) {
        double total = 0;
        ...
        try {
            ...
            for (String line : lines) {
                ...
                if (data[2].equals(accountId)
                        && data[5].equals(transactionType)
                        && getTransactionDate(data).equals(today)) {
    ...
    public double getTodayDepositTotal(String accountId) {
        return getTodayTransactionTotal(accountId, "DEPOSIT");
    }

    public double getTodayWithdrawTotal(String accountId) {
        return getTodayTransactionTotal(accountId, "WITHDRAW");
    }

    public double getTodayOwnTransferTotal(String accountId) {
        return getTodayTransactionTotal(accountId, "OWN-TRANSFER-WITHDRAW");
    }

    public double getTodayTransferTotal(String accountId) {
        return getTodayTransactionTotal(accountId, "TRANSFER-WITHDRAW");
    }

    public double getTodayOwnTransferDeposit(String accountId) {
        return getTodayTransactionTotal(accountId, "OWN-TRANSFER-DEPOSIT");
    }

    public double getTodayTransferDeposit(String accountId) {
        return getTodayTransactionTotal(accountId, "TRANSFER-DEPOSIT");
    }
}
